...
`setup.command` 安裝基本功能；如要額外安裝高質 Demucs AI 模式，就雙擊 `install-ai.command`。後者會先執行相同基本安裝，再自動識別 Apple Silicon 或 Intel Mac，並選擇相容嘅 PyTorch 套件。遇到 PyPI 502／503 或短暫網絡錯誤時，macOS 安裝器會延長 pip 逾時並自動重試。
